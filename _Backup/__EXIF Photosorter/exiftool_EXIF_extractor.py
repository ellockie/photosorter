		...
		if os.path.isfile(os.path.join(root, file)):
			...
			if file_extension in supported_image_extensions:
				...
				os.rename(
					...
					os.path.join(full_path_of(problematic_folder), UNSUP_EXT_subfolder_name, filename + file_extension)
				)
				# except:
	...
	if previous_critical_fail_counter < critical_fail_counter:
		...
		if image_info_object["orig_file_ext"].lower in special_raw_image_extensions:
			...
			os.rename(
				...
				os.path.join(full_path_of(CRW_conv_folder), image_info_object["orig_file_name"] + image_info_object["orig_file_ext"])
			)
			...
			os.rename(
				os.path.join(full_path_of(unsorted_folder), image_name),
				os.path.join(NOT_ENOUGH_INFO_subfolder_full_path, image_info_object["orig_file_name"] + image_info_object["orig_file_ext"])
			)
		return False
...
def rename_exif_file(exif_file_name, image_file_name):
	...
	try:
		os.rename(exif_file_name, os.path.join(full_path_of(unsorted_folder), image_file_name + exif_extension_str))
	except:
...
def get_the_destination_path(file_name, file_ext, source_img_path, iterator=0):
	...
	destination_exif_path = os.path.join(path_to_destination_photos, file_name + iterator_str(iterator) + exif_extension_str)
...
def _TASK_verify_if_folders_exist(all_folders):
	...
	UNSUP_EXT_subfolder_full_path = os.path.join(full_path_of(full_path_of(problematic_folder)), UNSUP_EXT_subfolder_name)
	...
	EMPTY_FILES_subfolder_full_path = os.path.join(full_path_of(full_path_of(problematic_folder)), EMPTY_FILES_subfolder_name)
	...
	NOT_ENOUGH_INFO_subfolder_full_path = os.path.join(full_path_of(full_path_of(problematic_folder)), NOT_ENOUGH_INFO_subfolder_name)
	...
	DUPLICATE_FILE_NAMES_subfolder_full_path = os.path.join(full_path_of(full_path_of(problematic_folder)), DUPLICATE_FILE_NAMES_subfolder_name)
...
def _TASK_move_old_exif_files():
	...
	for exif_file_name in yield_pattern_matching_files_from_location("*._exif", full_path_of(unsorted_folder)):
		...
		moved_old_exif_dest_file_name = os.path.join(full_path_of(problematic_old_EXIF_folder), exif_file_name[1])
...
def _TASK_remove_empty_image_files():
	...
	for image_file_name_with_path in yield_image_files_from_location(full_path_of(unsorted_folder), include_paths=True):
		...
		if os.path.getsize(image_file_name_with_path) == 0:
			...
			moved_empty_file_name_destination = os.path.join(EMPTY_FILES_subfolder_full_path, image_file_name)
...
def _TASK_collect_info_from_EXIF_files():
	...
	for image_file_name in yield_image_files_from_location(full_path_of(unsorted_folder), include_paths=False):
		...
		image_file_name_only, image_file_extension = os.path.splitext(image_file_name)
		...
		if image_file_extension not in special_raw_image_extensions:
			...
			exif_file_name = os.path.join(full_path_of(unsorted_folder), image_file_name_only + exif_extension_str)
			...
			if extraction_success:
				...
				moved_exif_file_name = os.path.join(NOT_ENOUGH_INFO_subfolder_full_path, os.path.splitext(image_file_name)[0] + exif_extension_str)
...
def _TASK_rename_and_move_images_and_EXIF_files():
	...
	for img_data in file_info_array:
		...
		try:
			os.rename(
				os.path.join(full_path_of(unsorted_folder), original_file_name + original_file_ext + exif_extension_str),
		...
		for file_format in (supported_image_extensions + supported_non_image_extensions):
			...
			for image_file_name in yield_pattern_matching_files_from_location("*" + file_format, full_path_of(result_folder)):
				...
				if "._exif" in image_file_name[1]:
					EXIF_folder_destination = os.path.join(create_date_folder(image_file_name[1]), EXIF_subfolder_name)
					# check_if folder has been created
					...
					try:
						os.rename(
							image_file_name[0],
							os.path.join(EXIF_folder_destination, image_file_name[1])
						)
					except:
						...
						try:
							os.rename(
								image_file_name[0],
								os.path.join(os.path.join(DUPLICATE_FILE_NAMES_subfolder_full_path, image_file_name[1]))
							)
						except:
					...
					RAW_folder_destination = os.path.join(create_date_folder(image_file_name[1]), RAW_subfolder_name)
					# check_if folder has been created
					...
					os.rename(
						image_file_name[0],
						os.path.join(RAW_folder_destination, image_file_name[1])
					)
				else:
					try:
						os.rename(
							image_file_name[0],
							os.path.join(create_date_folder(image_file_name[1]), image_file_name[1])
						)
					except:
