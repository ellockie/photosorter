	...
	for file in os.listdir(root):
		...
		if os.path.isfile(os.path.join(root, file)):
			...
			if file_extension in supported_image_extensions:
				...
				UNSUP_EXT_subfolder_name_full_path = os.path.join(full_path_of(full_path_of(problematic_folder)), UNSUP_EXT_subfolder_name)
...
def _TASK_verify_if_folders_exist(all_folders):
	...
	if missing_folder_count:
		...
		if len(missing_folders) == len(all_folders):
			...
			answer = raw_input("    Use the current working directory? (Enter=yes): ")
			if answer == "":
				root_folder_path = os.path.join(os.getcwd(), "__PROC_PHOTOS")
				print "\n  Will use the following path: ", root_folder_path, "\n"
			else:
				answer = raw_input("  Continue with original root folder path? (Enter=yes): ")
				if answer != "":
					print "\n\tQuitting\n"
					quit()
...
def _TASK_show_stats():
	...
	if (fail_counter > 0) or (duplicate_counter > 0):
		if fail_counter > 0:
			print("\n\tEncountered " + str(fail_counter) + " problems with " + "some files")
		if critical_fail_counter > 0:
			print("\n\tEncountered " + str(critical_fail_counter) + " CRITICAL problems with " + "some files")
