...
- **Add a single-call UI launcher (2026-06-15)**: one call (`_photosorter_ui.bat` / `main.py --ui`) starts the dashboard server, auto-opens the browser (reusing a live server if already running), runs the pipeline in a single click (`/api/pipeline/run`), and stops the server/launcher from a dashboard button (`/api/server/shutdown`) while remaining re-runnable until stopped.
